...
CGO_ENABLED=0 GOOS=linux GOARCH=$(go env GOARCH) go build -v \
  -ldflags "-s -w \
...
docker build \
  --tag docker.io/gyuho/hello-world:${RELEASE_VERSION} \
  --file ./Dockerfile .

docker push docker.io/gyuho/hello-world:${RELEASE_VERSION}
